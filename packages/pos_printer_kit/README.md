...
- if printer is powered off after connection, next write/connect failure marks state as disconnected
...
- optional manual `refreshConnectionStatus()` API (on-demand checks only)
